...
namespace TEngine.Core.Network
{
    ...
    public sealed class MessageDispatcherSystem : Singleton<MessageDispatcherSystem>
    {
        ...
        protected override void OnLoad(int assemblyName)
        {
            ...
            // 如果编译符号TENGINE_NET存在，遍历所有实现了IRouteMessageHandler接口的类型，创建实例并添加到_routeMessageHandlers字典中
#if TENGINE_NET
        ...
        protected override void OnUnLoad(int assemblyName)
        {
            ...
            // 如果编译符号TENGINE_NET存在，移除程序集对应的路由消息处理器信息
#if TENGINE_NET
        ...
        // 如果编译符号TENGINE_NET存在，定义处理路由消息的方法
#if TENGINE_NET
        /// <summary>
